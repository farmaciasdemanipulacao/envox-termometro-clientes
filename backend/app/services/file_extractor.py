...
Tipos suportados:
  - image (JPG, PNG, WEBP, GIF)  → Vision API (Groq llama-4-scout / OpenAI gpt-4o-mini)
  - document PDF                  → pypdf (texto nativo)
  - document DOCX                 → python-docx
  - document TXT                  → UTF-8 decode
  - document XLSX                 → openpyxl
  - video (MP4, etc.)             → ffmpeg extrai áudio → Whisper

Quando nenhuma chave de API estiver configurada, retorna None para tipos que dependem de IA.
"""
import asyncio
import base64
import io
...
async def _describe_image(b64: str, mimetype: str) -> str | None:
    ...
    return await loop.run_in_executor(None, _sync_vision, b64, mimetype, openai_key, groq_key)


def _sync_vision(b64: str, mimetype: str, openai_key: str, groq_key: str) -> str | None:
    ...
    try:
        ...
        if not text:
            return None
        logger.info("vision_ok", chars=len(text), model=model)
        return f"[Imagem] {text}"
    except Exception as e:
        logger.error("vision_api_failed", error=str(e), model=model)
        return None
